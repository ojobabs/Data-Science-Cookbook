...
Whenever I have overview map I want to be able to show a rectangle that represents the current extent of the mine map.  To do this are ArcGIS pro can use the extent indicators. `arcpy` cannot be used to insert an extend indicator. And so we will do that by clicking on the map frame in the layout and use the insert tab on the ribbon to choose an extent indicator for the Main map from.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA3MTgwMzY2MCwxNDc0MzU0NTA1LDkzMj
