...
> ~~Update: probably, this above approach won't work because we need to add the Map Frame (the actual map) to the layout by using the ribbon. There is no way to use code for this. ~~ It is going to work because if you change the layer on the main map it changes on the layout too.
...
```
Now make that "Overview Map Frame" map frame be 80 millimeters wide and 50 millimeters high
```python
OverviewMapFrame.elementWidth = 80
OverviewMapFrame.elementHeight = 40
```
Let's save the current project:
```python
aprx.save()
```
...
Let's now use the office layer on the Overview Map to query and get only S51 office and all satellite office under it. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ4NTM5MzUxMiwzMjE5OTQ5ODIsLTExNT
